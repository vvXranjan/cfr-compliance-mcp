...
class Citation:
    ...
    def _build_browse_url(self) -> str:
        ...
        Built from eCFR's documented browse-page URL pattern
        (`ecfr.gov/current/title-X/part-Y/section-X.Y`). Treat it as a
        convenience link for humans reviewing a compliance report, not
        as a value the system depends on for correctness — the
        authoritative identifier is the (title, part, section, date)
        tuple, not this URL.
        """
        ...
        if self.section is None:
            return f"{base}/part-{self.part}"
        section_id = (
            self.section
            if "." in self.section
            else f"{self.part}.{self.section}"
        )
...
def _find_all(scope: ET.Element, expected_type: str, heading_re: re.Pattern[str], requested_num: str) -> list[ET.Element]:  # noqa: E501
...
def _find_target_node(
    ...
    if part is not None:
        ...
        if not matches:
            # Section endpoint returns a SECTION as the root,
            # so there is no PART node.
            if section is None:
    ...
    if section is not None:
        # eCFR returns section numbers as "1910.1200", while callers
        # pass "1200". Qualify it when needed.
        requested_section = (
            f"{resolved_part or part}.{section}"
            if "." not in section and (resolved_part or part) is not None
            else section
        )
        ...
        if not matches:
            # Section-scoped endpoint:
            # root itself is already the SECTION element.
            if (
                _node_number(root, _SECTION_TYPE, _SECTION_HEAD_RE)
                == requested_section
            ):
                target = root
            else:
        ...
        resolved_section = (
            _node_number(target, _SECTION_TYPE, _SECTION_HEAD_RE)
